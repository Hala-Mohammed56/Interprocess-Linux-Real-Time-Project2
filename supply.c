...
int* get_store_field(Store* store, int index) {
    int* fields[N_ITEMS] = {
        ...
        &store->sugar,  // إذا عندك sugar و salt مفصولين، عدل هون
        &store->sweet_items, &store->cheese, &store->salami
    };
    return fields[index];
}
        ...
        if (*val <= 0) {
            if (!states[i].waiting) {
                ...
                if (now - states[i].wait_start >= WAIT_SECONDS) {
                    ...
                    *val = new_stock;
                    printf("Restocked %s to %d after wait.\n", item_names[i], new_stock);
                    states[i].waiting = 0;
                }
            }
        } else {
...
int main() {
    ...
    int shmid = shmget(SHM_KEY, sizeof(BakerySharedData), IPC_CREAT | 0666);
    if (shmid == -1) { perror("shmget"); return 1; }

    BakerySharedData* shared = (BakerySharedData*)shmat(shmid, NULL, 0);
